...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkAllLoaded() {
        if (splineElementsLoaded >= totalSplineElements) {
            ...
            if ((window as any).updatePreloaderProgress) {
                (window as any).updatePreloaderProgress(100);
            }
        }
    }
    ...
    if (heroSpline) {
        heroSpline.addEventListener('load', () => {
            ...
            if ((window as any).updatePreloaderProgress) {
                (window as any).updatePreloaderProgress(50 + (splineElementsLoaded / totalSplineElements) * 50);
            }
            checkAllLoaded();
        });
    }
